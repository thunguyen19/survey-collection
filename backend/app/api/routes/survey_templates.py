...
@router.get(
    ...
    response_model=SurveyTemplatesPublic,
)
def read_survey_templates(
    session: SessionDep, current_user: CurrentUser, skip: int = 0, limit: int = 100
) -> Any:
    """
    Retrieve survey templates for the current user's organization. Admin access only for MVP.
    """
...
def read_active_survey_templates(
    ...
    """
    Retrieve only active survey templates for the current user's organization. Admin access only for MVP.
    """
    count_statement = select(func.count()).select_from(SurveyTemplate).where(
        SurveyTemplate.organization_id == current_user.organization_id,
...
def read_survey_template(
    ...
    """
    Get survey template by ID. Admin access only for MVP.
    """
